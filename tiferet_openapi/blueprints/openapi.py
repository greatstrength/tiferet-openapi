...
def create_openapi_request_context(interface_id: str,
    ...
    '''
    Compose an OpenApiRequestContext for a feature execution.

    Mirrors core.create_request_context but constructs an
    OpenApiRequestContext so handle_response serializes Pydantic BaseModel
    results before they reach build_response.
...
def build_openapi_session_context(app_session: AppSession,
    ...
    Parallel to build_cli_session_context: builds the app service container
    and resolver, then composes OpenApiSessionContext with the three OpenAPI
    event collaborators. Defaults create_request_handler to
    core.create_request_context for parity with the framework default;
    callers wanting OpenApiRequestContext's BaseModel-serialization behavior
    should supply create_openapi_request_context instead. This is a helper,
    not a full build_app-shaped blueprint, since tiferet-openapi is a library
    that tiferet-flask/tiferet-fast extend further with their own web-
    framework request/response glue.
    ...
    :param create_request_handler: The request-construction handler; defaults
        to core.create_request_context when omitted.
